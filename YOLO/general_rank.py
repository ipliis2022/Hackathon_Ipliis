...
def non_max_suppression(prediction,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            print('best class only')
            conf, j = x[:, 5:].max(1, keepdim=True)
            i_2, j_2 = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box, conf, j.float()), 1)[conf.view(-1) > conf_thres]

        i_2, j_2 = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).T
        candi = torch.cat((box_2[i_2], y[i_2, :]), 1)
        tensor_candid, index_candid = torch.sort(candi[:, j+4], dim=0, descending=True)
        best_bbox = candi[index_candid[0,0], :]
